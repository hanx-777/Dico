...
def _rank_utility(curve: Any, rank: int) -> float:
    if isinstance(curve, Mapping):
        if rank not in curve:
            raise ValueError(f"Utility curve missing rank {rank}")
        return float(curve[rank])
    if isinstance(curve, (list, tuple)):
        if rank >= len(curve):
            raise ValueError(f"Utility curve length {len(curve)} cannot provide rank {rank}")
        return float(curve[rank])
    raise TypeError(f"Unsupported utility curve type: {type(curve)!r}")


def solve_rank_dp(
    module_utility_curves: Mapping[str, Any],
    module_dims: Mapping[str, Mapping[str, Any]],
    target_budget: int,
    r_min: int,
    r_max: int,
    ...
    """Solve CovRA's real-cost integer rank allocation with multi-choice DP.

    Each module chooses one integer rank in ``[r_min, r_max]``. Costs are true
    LoRA trainable parameters per rank, ``d_in + d_out``. The solver never
    chooses an over-budget allocation unless the minimum feasible rank already
    exceeds the target budget; in that case the returned diagnostics make the
    infeasibility explicit.
    """

    module_names = list(module_dims.keys())
    if int(r_min) > int(r_max):
        raise ValueError(f"r_min must be <= r_max, got r_min={r_min}, r_max={r_max}")
    missing = [name for name in module_names if name not in module_utility_curves]
    if missing:
        raise KeyError(f"Missing utility curves for modules: {missing}")

    costs = {name: module_rank_cost(module_dims[name]) for name in module_names}
    min_allocation = {name: int(r_min) for name in module_names}
    min_budget = compute_total_lora_params(min_allocation, module_dims)
    lower_bound = int(math.ceil(float(eta) * int(target_budget)))

    if min_budget > int(target_budget):
        info = _budget_info(
            min_allocation,
            int(target_budget),
            module_dims,
        ...
        module_logs = [
            {
                ...
                "final_rank": int(r_min),
                "utility": _rank_utility(module_utility_curves[name], int(r_min)),
            }
            for name in module_names
        ]
        return WeightedAllocationResult(
            ...
            diagnostics={
                "solver": "dp",
                "budget_lower_bound": lower_bound,
                "budget_lower_bound_feasible": False,
                "fallback": "minimum_rank_exceeds_target",
    ...
    for name in module_names:
        ...
        cost = costs[name]
        curve = module_utility_curves[name]
        for used_budget, (utility, ranks) in states.items():
            for rank in range(int(r_min), int(r_max) + 1):
                new_budget = used_budget + rank * cost
                if new_budget > int(target_budget):
                    continue
                new_utility = utility + _rank_utility(curve, rank)
                new_ranks = ranks + (int(rank),)
                existing = next_states.get(new_budget)
                if existing is None or (new_utility, new_ranks) > (existing[0], existing[1]):
                    next_states[new_budget] = (new_utility, new_ranks)
        states = next_states
        if not states:
            break
    ...
    if not states:
        ...
        return WeightedAllocationResult(
            ...
            diagnostics={
                ...
                "budget_lower_bound": lower_bound,
                "budget_lower_bound_feasible": False,
                "fallback": "no_under_budget_state",
            },
        )

    feasible_budgets = [budget for budget in states if lower_bound <= budget <= int(target_budget)]
    lower_bound_feasible = bool(feasible_budgets)
    if feasible_budgets:
        best_budget = max(
            feasible_budgets,
            key=lambda budget: (states[budget][0], budget, states[budget][1]),
        )
        fallback = None
    else:
        best_budget = max(
            states,
            key=lambda budget: (budget, states[budget][0], states[budget][1]),
        )
        fallback = "best_under_budget"

    best_utility, best_ranks = states[best_budget]
    allocation = {name: int(rank) for name, rank in zip(module_names, best_ranks)}
    info = _budget_info(
        allocation,
        int(target_budget),
    ...
    if fallback and info.warning is None:
        info = replace(info, warning=fallback)
    ...
    module_logs = [
        {
            ...
            "utility": _rank_utility(module_utility_curves[name], allocation[name]),
        }
        for name in module_names
    ]
    return WeightedAllocationResult(
        ...
        diagnostics={
            "solver": "dp",
            "budget_lower_bound": lower_bound,
            "budget_lower_bound_feasible": lower_bound_feasible,
            "fallback": fallback,
            "objective_value": best_utility,
            "budget_state_count": len(states),
        },
    )
...
def allocate_by_evidence_aware_utility(
    ...
    This utility converts selected evidence utilities into an integer allocation
    under the same parameter-budget accounting used by the v0.3 procurement path.
    """
...
def allocate_by_directional_evidence(
    ...
    """Allocate ranks by directly purchasing certified direction evidence.

    This is the static DiCo-eta path described in the paper draft: selected
    directions are budgeted as items with module-specific single-rank costs.
    If certified directions run out before the eta lower bound, optional
    evidence relaxation fills remaining budget using each module's average
    certified density.
    """
    ...
    selected_utilities: dict[str, list[float]] = {name: [] for name in module_names}

    selected_atoms = []
    for row in atom_logs:
        module_name = str(row.get("module_name"))
        if module_name not in module_dims or not bool(row.get("selected", False)):
            continue
        utility = max(0.0, float(row.get("utility", 0.0) or 0.0))
        selected_utilities.setdefault(module_name, []).append(utility)
        selected_atoms.append((module_name, int(row.get("atom_index", row.get("atom_id", 0)) or 0), utility))
    ...
    for module_name, _atom_index, utility in sorted(
        selected_atoms,
        key=lambda item: (
            item[2] / max(costs[item[0]], 1) ** float(beta),
            item[2],
            -costs[item[0]],
            item[0],
            -item[1],
        ...
        if allocation[module_name] >= int(r_max):
            continue
        if total() + costs[module_name] > int(target_budget):
            continue
    ...
    for name in module_names:
        ...
        avg_density[name] = avg_utility / max(costs[name], 1) ** float(beta)
    ...
    while total() < target_min:
        ...
        if not candidates:
            break
        best = max(candidates, key=lambda name: (avg_density[name], -costs[name], name))
        allocation[best] += 1
        ...
        module_logs.append(
            {
                ...
                "final_parameter_count": rank * costs[name],
                "final_budget": rank * costs[name],
