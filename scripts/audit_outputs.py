...
def _audit_config(
    ...
    if expected["method"] in PREALLOC_METHODS:
        ...
        expected_aggregation = "weighted_log" if atom_mode == "svd" else "weighted_topk"
        if pre_cfg.get("aggregation_mode") != expected_aggregation:
            critical.append(f"{experiment}: preallocation.aggregation_mode must be {expected_aggregation}")
...
def _audit_preallocation_metadata(
    ...
    expected_aggregation = "weighted_log" if atom_mode == "svd" else "weighted_topk"
    if aggregation_mode != expected_aggregation:
        critical.append(f"{experiment}: missing/invalid aggregation_mode={aggregation_mode!r}, expected {expected_aggregation!r}")
    if atom_norm != "none":
    ...
    if atom_mode == "svd" and not (initial_payload.get("profile_norm_mode") or metadata.get("profile_norm_mode")):
        critical.append(f"{experiment}: svd atom_mode requires profile_norm_mode diagnostics")
    if not metadata:
